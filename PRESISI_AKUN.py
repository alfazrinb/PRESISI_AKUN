...
risiko_model = pickle.load(open("Detection_risk_modelling.sav", "rb")) 
...
if (selected == 'Prediksi Risiko Akun'):
    ...
    Opini = st.selectbox('Opini',['WDP','WTP'])
    ...
    IR = st.selectbox('Inherent Risk', ['High','Low','Medium'])
    ...
    if st.button('Submit'):
        ...
        if (prediksi_risiko[0] == 0):
            prediksi = f'Akun terkait Pada Entitas tersebut Memiliki Risiko High, dan Strategi Pemeriksaan Terbatas'
        elif (prediksi_risiko[0] == 2):
            prediksi = f'Akun Terakit Pada Entitas Tersebut Memiliki Risiko Low, dan Strategi Pemeriksaan Mendalam'
        else:
            prediksi = f'Akun Terkait Pada Entitas Tersebut Memiliki Risiko Medium, dan Strategi Pemeriksaan Menengah'
...
if (selected == 'Ekpsloratory Data Analysis'):
    ...
    def show_histogram(data, numeric_column, categoric_column):
        st.header(f'Histogram {numeric_column} berdasarkan {categoric_column}')

        fig, ax =plt.subplots()

        for category in data[categoric_column].unique():
            subset = data[data[categoric_column]==category]
            subset[numeric_column].plot(kind='hist', 
                                            bins=20, 
                                            alpha=0.5, 
                                            label=str(category), 
                                            ax=ax)
                
        #menampabahkan legend
    ...
    def main():
        ...
        #menampilkan histogram
        selected_column = st.selectbox('Pilih kolom untuk melihat distribusi:', data.select_dtypes(include='number').columns)
        show_histogram(data, selected_column, 'Risiko')
